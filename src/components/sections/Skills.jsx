...
const Skills = () => {
  ...
  // Map category colors to text colors for skill badges
  const textColors = {
  ...
  return (
    <section id="skills" className="space-y-4 md:space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {categories.map((cat) => (
          <div 
            key={cat.title} 
            className={`relative p-5 md:p-6 rounded-3xl bg-gradient-to-br from-${cat.color}-50/10 via-${cat.color}-100/20 to-${cat.color}-200/20 
              backdrop-blur-lg border border-${cat.color}-200/20 overflow-hidden group hover:scale-105 transition-transform duration-500 shadow-lg hover:shadow-2xl`}
          >
